...
def inputD(x,v=0):
	while 1:
		try:
			...
			tampil('\n\rm[!]Batal')
			keluar() __future__ import print_function
import platform,os
def tampil(x):
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!]Masukan jarimu Bro...')
				continue
		else:
			if len(a) == 0:
				tampil('\rm[!]Masukan dengan pelan')
				continue
			else:
				break
	return a
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rh[*]Selamat datang \rk%s\n\rh[*]Semoga esok adalah hari kematian mu....'%nama)
		log = 1
...
#       \rmIzin Kepada pemilik Sebelum Penggunaan           \rk#
###########################################################''')
